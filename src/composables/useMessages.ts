...
export function useMessages(
  conversation?: Ref<ConversationData | null>,
  ...
  const normalizeMessage = (msg: any): ChatMessage => {
    ...
    const isDateMessage =
      ...
      dateRegex.test(msg.message_content);
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      const {
        ...
        hasMore,
        nextBeforeId: nextId,
      } = await ChatService.loadHistoryMessages(chatConv);
  ...
  const loadOlderMessages = async () => {
    ...
    try {
      ...
      const {
        ...
        hasMore,
        nextBeforeId: nextId,
      } = await ChatService.loadOlderMessages(conv.uuid, beforeId, 20);
      // Prepend older messages to the beginning of the array
      messages.value = [
        ...(older || []).map(normalizeMessage),
        ...messages.value,
      ];
      hasMoreMessages.value = hasMore;
      nextBeforeId.value = nextId;
